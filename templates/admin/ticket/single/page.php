...
if(!aiosc_is_ticket($ticket) || (!$aiosc_user->can('view_ticket',array('ticket_id'=>$ticket))) || ($edit_mode && !$aiosc_user->can('edit_ticket',array('ticket_id'=>$ticket)))) {
    ...
    if($edit_mode) {
        ...
        $visibility_check = "<label><input type='checkbox' name='is_public' ".checked($ticket->is_public, true, false)."> ".__('Publica','aiosc')."</label>";
    }
?>
    ...
    <?php if($edit_mode) : ?>
        <div class="aiosc-edit-mode-notice"><p><?php _e('Actualmente estás en <code>MODO EDITAR TICKET</code>, por lo que no puedes publicar respuestas.','aiosc')?></p></div>
...
<div class="wrap aiosc-grid-holder">
    <div class="aiosc-ticket-widgets">
        <a href="#" id="aiosc-sidebar-toggler"><?php _e('Más Detalles','aiosc')?></a>
        <div class="aiosc-window">
            <h2 class="page-title"><?php _e('Detalles','aiosc') ?>
            ...
            <table class="aiosc-ticket-details-table">
                ...
                <tr>
                    ...
                    <td>
                        <?php if($edit_mode && $ticket->status == 'closed') : ?>
                            ...
                            <label><input type="checkbox" name="reopen" value="1" /> <?php echo $ticket->status_name?> - <?php _e('¿Reabrir?','aiosc')?> </label>
                        </span>
                ...
                <tr><th><?php _e('Autor','aiosc')?>:</th><td>
                        <?php if($edit_mode) : ?>
                            <?php echo $author_select; ?>
                        <?php else : ?>
                            <?php if(!empty($author_url)) : ?>
                                <a href="<?php echo $author_url?>" target="_blank" title="<?php _e('Ver perfil','aiosc')?>"><?php echo $author->display_name; ?></a>
                            <?php else : ?>
                ...
                <tr><th><?php _e('Operador','aiosc')?>:</th><td>
                        <?php if(!$edit_mode) : ?>
                            <?php if(!empty($operator_url)) : ?>
                                <a href="<?php echo $operator_url?>" target="_blank" title="<?php _e('Ver perfil','aiosc')?>"><?php echo $operator->display_name; ?></a>
                            <?php else : ?>
                ...
                <tr><th><?php _e('Visibilidad','aiosc')?>:</th>
                    <td>
                        <?php if(!$edit_mode) : ?>
        <?php echo ($ticket->is_public)?__('Publica','aiosc'):__('Privada','aiosc')?>
        <?php else : ?>
        <?php echo $visibility_check ?>
        <?php endif; ?>
                    </td>
                </tr>
                <tr><th><?php _e('Fecha de Creación','aiosc')?>:</th><td>
                ...
                <?php if(strtotime($ticket->date_open) > 0) : ?>
                    <tr><th><?php _e('Abierto desde','aiosc')?>:</th><td>
                ...
                <?php if(strtotime($ticket->date_closed) > 0) : ?>
                    <tr><th><?php _e('Cerrado desde','aiosc')?>:</th><td>
        ...
        <?php if(is_array($ticket->attachment_ids) && $aiosc_user->can('reply_ticket',array('ticket_id'=>$ticket))) : ?>
            <div class="aiosc-window">
                <h2 class="page-title"><?php _e('Archivos adjuntos','aiosc') ?>
                <div class="aiosc-separator"></div></h2>
        ...
        <?php if($aiosc_user->can('staff') ||
            ...
            <div class="aiosc-window">
                <h2 class="page-title"><?php _e('Acciones','aiosc') ?>
                    <div class="aiosc-separator"></div></h2>
                <?php if(!$edit_mode) : ?>
                    ...
                    <?php endif; ?>
                    <?php if($aiosc_user->can('edit_ticket',array('ticket_id'=>$ticket))) : ?>
                        <button type="button" class="button" onclick="window.location.href='<?php echo aiosc_get_page_ticket_preview($ticket,true)?>'"><?php _e('Modo de Edición','aiosc')?></button>
                    <?php endif; ?>
                    ...
                    <?php endif; ?>
                <?php else : ?>
                    <button type="submit" class="button button-primary edit-button-submit"><?php _e('Guardar Edición','aiosc')?></button>
                    <button type="button" class="button" onclick="window.location.href='<?php echo aiosc_get_page_ticket_preview($ticket,false)?>'"><?php _e('Salir','aiosc')?></button>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        <?php if($ticket->status == 'closed') : ?>
            <div class="aiosc-window">
                <h2 class="page-title"><?php _e('Nota de Cierre','aiosc') ?>
                    <div class="aiosc-separator"></div></h2>
    ...
    <div class=" aiosc-ticket-main">
        <div class="aiosc-window">
            <h2 class="page-title">
                <?php if(!$edit_mode) : ?>
        <?php echo !empty($ticket->subject)?$ticket->subject:__('Ticket sin titulo','aiosc'); ?>
        <?php else : ?>
        <?php echo $subject_textbox ?>
        <?php endif; ?>
                <span><?php printf(__('Creado por <a href="%s">%s</a> en %s a %s','aiosc'),aiosc_get_user_url($author->ID,'#'),
                        $author->display_name,
        ...
        <div class="aiosc-eot"><span><?php _e('Fin de contenido','aiosc')?></span></div>
