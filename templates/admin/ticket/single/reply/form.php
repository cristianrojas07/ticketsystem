...
<div class="aiosc-window aiosc-reply-form">
    <?php if(!$ticket->awaiting_reply) : ?>
        <span id="aiosc-expand-reply-form"><?php _e('Haga clic aquí para mostrar el formulario de respuesta.', 'aiosc')?></span>
    <?php endif; ?>
    <div class="aiosc-reply-form-content" <?php if(!$ticket->awaiting_reply) : ?>style="display: none;"<?php endif; ?>>
        <h2 class="page-title page-title-sm"><?php _e('Publicar Nueva Respuesta','aiosc') ?></h2>
        ...
        <form id="aiosc-form" action="<?php echo get_admin_url()?>admin-ajax.php" method="post" enctype="multipart/form-data">
            ...
            <?php if($aiosc_user->can('staff')) :
                ...
                if(!empty($premades)) :
                    ...
                    <div id="aiosc-premade-responses">
                        <select >
                            <option value="0"><?php _e('- Seleccione Respuesta Pre-hecha -','aiosc')?></option>
                            <?php
                            if(!empty($my_premades)) : ?>
                                <optgroup label="<?php _e('Mis Respuestas','aiosc')?>">
                                    <?php
                            ...
                            if(!empty($shared_premades)) : ?>
                                <optgroup label="<?php _e('Compartido por otros','aiosc')?>">
                                    <?php
                            ...
                            <li class="aiosc-uploader-browse">
                                <i class="dashicons mainicon dashicons-welcome-add-page"></i>
                                       <span class="aiosc-uploader-file-name">
                                            <a href="#"><?php _e('Adjuntar archivo', 'aiosc')?></a>
                                       </span>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="aiosc-separator"></div>
            <div class="cholder">
                <input type="submit" value="<?php _e('Enviar Respuesta','aiosc')?>" class="button button-primary" />
            </div>
        </form>
    </div>
    <div class="aiosc-loading-holder"><div class="aiosc-loading-bar"><span><?php _e('Enviando respuesta...','aiosc')?></span></div></div>
</div>
<script>
