...
class aiosc_AttachmentManager {
    ...
    static function upload_attachments($files, $post_type='ticket') {
        try {
            ...
            if(!$aiosc_settings->get('allow_upload')) return aiosc_response(0,__('<strong>Error:</strong> Las cargas de archivos están deshabilitadas por el sistema.','aiosc'));
            if(!$aiosc_user->can('upload_files')) return aiosc_response(0,__("<strong>Error:</strong> No tienes los permisos adecuados para subir archivos.",'aiosc'));
            ...
            if($max_files < count($files)) {
                if($post_type == 'ticket')
                    return aiosc_response(0,sprintf(__('<strong>Error:</strong> El máximo de archivos que puede cargar mientras crea el ticket es %d, pero usted adjuntó %d.','aiosc'),$max_files,count($files)));
                else
                    return aiosc_response(0,sprintf(__('<strong>Error:</strong> El máximo de archivos que puede cargar mientras publica la respuesta es %d, pero adjuntó %d.','aiosc'),$max_files,count($files)));
            }
            ...
            for($i=0;$i<count($files);$i++) {
                ...
                if($e == UPLOAD_ERR_OK) {
                    if($f['size'] > ($max_size * 1024)) {
                        $errors[] = sprintf(__('<strong>Error:</strong> El archivo <code>%s</code> excede el límite máximo de tamaño de archivo.','aiosc'),$files[$i]['name']);
                    }
                }
                ...
                elseif($e == UPLOAD_ERR_INI_SIZE || UPLOAD_ERR_FORM_SIZE) {
                    ...
                    if($e != UPLOAD_ERR_OK) {
                        if(UPLOAD_ERR_NO_TMP_DIR)
							$errors[] = sprintf(__('<strong>Error:</strong> Archivo <code>%s</code> | No hay directorio temporal.','aiosc'),$files[$i]['name']);
						elseif(UPLOAD_ERR_CANT_WRITE)
							$errors[] = sprintf(__('<strong>Error:</strong> Archivo <code>%s</code> | No se puede escribir en el disco.','aiosc'),$files[$i]['name']);
						elseif(UPLOAD_ERR_PARTIAL)
							$errors[] = sprintf(__('<strong>Error:</strong> Archivo <code>%s</code> | Solo subido parcialmente','aiosc'),$files[$i]['name']);
						elseif(UPLOAD_ERR_EXTENSION)
							$errors[] = sprintf(__('<strong>Error:</strong> Archivo <code>%s</code> | Una extensión de PHP detuvo la carga del archivo.','aiosc'),$files[$i]['name']);

						else $errors[] = sprintf(__('<strong>Error:</strong> El archivo <code>%s</code> no se pudo cargar debido a una razón desconocida. | %s','aiosc'),$files[$i]['name'], $e);
                    }
                }
            }
            ...
            foreach($files as $file) {
                if(!self::is_valid_ext($file['name'])) $errors[] = sprintf(__('<strong>Error:</strong> El archivo <code>%s</code> tiene una extensión no válida.','aiosc'),$file['name']);
            }
            if(count($errors)) return aiosc_response(0,implode("<br>",$errors));
            ...
            if($upload_dir === false) return aiosc_response(0,__('<strong>Error:</strong> Los archivos no se pudieron cargar debido a una razón desconocida.','aiosc'));
            ...
            foreach($files as $file) {
                ...
                if(!move_uploaded_file($file['tmp_name'],$upload_dir."/".$enc_name.".".$ext)) $errors[] = sprintf(__('<strong>Error:</strong> El archivo <code>%s</code> no se pudo cargar debido a una razón desconocida.','aiosc'),$name);
                else {
                    $id = self::insert_into_db($name, $enc_name, $ext);
                    if($id < 1) {
                        $errors[] = sprintf(__('<strong>Error:</strong> El archivo <code>%s</code> no se pudo cargar debido a una razón desconocida.','aiosc'),$name);
            ...
            else return aiosc_response(1,sprintf(__('Total de %d archivos fueron cargados con éxito.','aiosc'),count($successes)),array('files'=>$successes));
        }
        catch(RuntimeException $e) {
            //something went wrong seriously...
            return aiosc_response(0,__('<strong>Error:</strong> Los archivos no se pudieron cargar debido a un error desconocido.','aiosc'));
        }
    }
    ...
    static function shortcode_downloader() {
        ...
        if(aiosc_is_attachment($att) && $att->_file_exists()) {
            ...
            else aiosc_log('[Ticket System Downloader] [Archivo ID: '.aiosc_pg('file_id').'] [Ticket ID: '.aiosc_pg('ticket_id').'] Se ha encontrado un archivo adjunto, pero el usuario no tiene permiso para descargarlo..');
        }
        else aiosc_log('[Ticket System Downloader] [Archivo ID: '.aiosc_pg('file_id').'] [Ticket ID: '.aiosc_pg('ticket_id').'] Archivo adjunto no encontrado.');
        exit;
    }
}
