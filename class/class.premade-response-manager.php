...
class aiosc_PremadeResponseManager {
    static function update_response($id = 0, $name, $content, $is_shared = true) {
        ...
        if(empty($name)) return aiosc_response(0,__('<strong>Error:</strong> La respuesta pre-hecha debe tener un nombre.','aiosc'));
        ...
        if(aiosc_is_premade_response($pr)) {
            if($pr->author_id != $aiosc_user->ID) return aiosc_response(0,AIOSC_PERMISSION_ERROR);
                ...
                if($name != $pr->name && !self::is_valid_name($name, $aiosc_user->ID))
                    return aiosc_response(0,sprintf(__('<strong>Error:</strong> Respuesta pre-hecha con &quot;%s&quot; Nombre ya existe. Para evitar confusiones más tarde, elija un nombre único.','aiosc'),$name));
            }
        }
        else {
        ...
        if(!aiosc_is_premade_response($pr)) {
            ...
            return aiosc_response(1,sprintf(__('Se ha creado la respuesta pre-hecha <strong>&quot;%s&quot;</strong>.','aiosc'),$name), array('response_id'=>$id));
        }
        else {
            //update
            ...
            return aiosc_response(1,sprintf(__('La respuesta pre-hecha <strong>&quot;%s&quot;</strong> se ha actualizado.','aiosc'),$name), array('response_id'=>$id));
        }
    }
    ...
    static function remove($ids=array()) {
        ...
        if($deleted != 1)
            return aiosc_response(1,sprintf(__('El total de <code>%s</code> respuestas pre-hechas se eliminaron con éxito.','aiosc'),$deleted));
        else
            return aiosc_response(1,__('La respuesta pre-hecha se ha eliminado correctamente.','aiosc'));
    }
    ...
    static function sharing($ids=array(),$share = true) {
        ...
        if($processed != 1) {
            if($share)
                return aiosc_response(1,sprintf(__('Se comparten el total de <code>%s</code> respuestas pre-hechas.','aiosc'),$processed));
            else
                return aiosc_response(1,sprintf(__('El total de <code>%s</code> respuestas pre-hechas se hacen privadas.','aiosc'),$processed));
        }

        else {
            if($share)
                return aiosc_response(1,__('La respuesta pre-hecha es compartida.','aiosc'));
            else
                return aiosc_response(1,__('La respuesta pre-hecha se hace privada.','aiosc'));
        }
    }
}
