...
class aiosc_DepartmentManager {
    ...
    static function update_department($id = 0, $name, $desc='', $ops = array(), $active = true, $bypass_user_check = false) {
        ...
        if(empty($name)) return aiosc_response(0,__('<strong>Error:</strong> Falta el nombre del departamento.','aiosc'));

        if(aiosc_is_department($dep)) {
            if($name != $dep->name && !self::is_valid_name($name)) return aiosc_response(0,__('<strong>Error:</strong> El nombre del departamento ya existe. Por favor elije otro.','aiosc'));
        }
        else {
            if(!self::is_valid_name($name)) return aiosc_response(0,__('<strong>Error:</strong> El nombre del departamento ya existe. Por favor elije otro.','aiosc'));
        }

        if(count($ops) < 1 && $active == true) {
            return aiosc_response(0,__('<strong>Error:</strong> Para estar <strong>activo</strong>, el departamento debe tener asignado al menos un operador activo.','aiosc'));
        }
        $errors = array();
        ...
        if(!aiosc_is_department($dep)) {
            ...
            return aiosc_response(1,sprintf(__('Se ha creado el departamento <strong>&quot;%s&quot;</strong>.','aiosc'),$name), array('department_id'=>$id));
        }
        else {
            //update
            ...
            return aiosc_response(1,sprintf(__('Se ha actualizado el departamento <strong>&quot;%s&quot;</strong>.','aiosc'),$name), array('department_id'=>$id));
        }
    }
    ...
    static function delete_department($id, $move_data_to=0) {
        ...
        if($id == $move_data_to) return aiosc_response(0,__('<strong>Error:</strong> El departamento al que mueve los datos no puede ser el mismo que el que intenta eliminar.','aiosc'));
        $dep = new aiosc_Department($id);
        if(!aiosc_is_department($dep)) return aiosc_response(0,sprintf(__('<strong>Error:</strong> El departamento con ID <code>%d</code> no existe.','aiosc'),$id));
        ...
        if(($tickets !== false || $ops !== false) && $move_data_to != 0 && !aiosc_is_department($new_dep)) return aiosc_response(0,__('<strong>Error:</strong> El departamento al que está moviendo los datos no existe.','aiosc'));
        ...
        if(aiosc_is_department($new_dep))
            return aiosc_response(1,sprintf(__('Departamento &quot;%s&quot; fue eliminado exitosamente El total de %d tickets y %d operadores se movieron a &quot;%s&quot; departamento.','aiosc'),
                $dep->name, $tickets_changed, $ops_moved, @$new_dep->name));
        else
            return aiosc_response(1,sprintf(__('Department &quot;%s&quot; fue eliminado exitosamente.','aiosc'),$dep->name));
    ...
    static function activate_departments($ids, $activate=true) {
        ...
        if($y > 0) {
            if(aiosc_enumToBool($activate) === true)
                return aiosc_response(1,sprintf(__('Total de %d departamentos fueron activados exitosamente.','aiosc'),$y));
            else
                return aiosc_response(1,sprintf(__('Total de %d departamentos fueron desactivados exitosamente.','aiosc'),$y));
        }
        else return aiosc_response(0,__('No se seleccionaron departamentos por lo que no se tomó ninguna acción.','aiosc'));
    }
    ...
    static function install() {
        ...
        if(!is_array($deps)) {
            ...
            self::update_department(0,
                __('Preguntas Generales','aiosc'),
                __('Este es el departamento predeterminado creado en la instalación de Ticket System.','aiosc'),
