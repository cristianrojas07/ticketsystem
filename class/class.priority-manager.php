...
class aiosc_PriorityManager {
    ...
    static function update_priority($id = 0, $name, $desc='', $level = 0, $color = null, $active = true, $bypass_user_check=false) {
        ...
        if(empty($name)) return aiosc_response(0,__('<strong>Error:</strong> Falta el nombre de prioridad.','aiosc'));

        $pri = new aiosc_Priority($id);

        if(aiosc_is_priority($pri)) {
            if($name != $pri->name && !self::is_valid_name($name)) return aiosc_response(0,__('<strong>Error:</strong> El nombre de prioridad ya existe. Por favor elije otro.','aiosc'));
        }
        else {
            if(!self::is_valid_name($name)) return aiosc_response(0,__('<strong>Error:</strong> El nombre de prioridad ya existe. Por favor elije otro.','aiosc'));
        }
        $errors = array();
        ...
        if(!aiosc_is_priority($pri)) {
            ...
            return aiosc_response(1,sprintf(__('Priority <strong>&quot;%s&quot;</strong> ha sido creado.','aiosc'),$name), array('priority_id'=>$id));
        }
        else {
            //update
            ...
            return aiosc_response(1,sprintf(__('Priority <strong>&quot;%s&quot;</strong> ha sido actualizado.','aiosc'),$name), array('priority_id'=>$id));
        }
    }
    ...
    static function delete_priority($id, $move_data_to=0) {
        ...
        if($id == $move_data_to) return aiosc_response(0,__('<strong>Error:</strong> La prioridad a la que está moviendo los datos no puede ser la misma que la que intenta eliminar.','aiosc'));
        $pri = new aiosc_Priority($id);
        if(!aiosc_is_priority($pri)) return aiosc_response(0,sprintf(__('<strong>Error:</strong> La prioridad con ID <code>%d</code> no existe.','aiosc'),$id));

        $tickets = aiosc_TicketManager::get_tickets(aiosc_TicketManager::get_ticket_query(array('priority_id'=>$id)));

        $new_pri = new aiosc_Priority($move_data_to);

        if($tickets !== false && $move_data_to != 0 && !aiosc_is_priority($new_pri)) return aiosc_response(0,__('<strong>Error:</strong> La prioridad a la que está moviendo los datos no existe.','aiosc'));
        ...
        if(aiosc_is_priority($new_pri))
            return aiosc_response(1,sprintf(__('Prioridad &quot;%s&quot; fue eliminado exitosamente El total de %d tickets se movió a &quot;%s&quot; prioridad.','aiosc'),
                $pri->name, $tickets_changed, @$new_pri->name));
        else
            return aiosc_response(1,sprintf(__('Prioridad &quot;%s&quot; fue eliminado exitosamente','aiosc'),$pri->name));
    }
    static function activate_priorities($ids, $activate=true) {
        ...
        if($y > 0) {
            if(aiosc_enumToBool($activate) === true)
                return aiosc_response(1,sprintf(__('El total de %d prioridades se activaron con éxito.','aiosc'),$y));
            else
                return aiosc_response(1,sprintf(__('El total de %d prioridades se desactivó con éxito.','aiosc'),$y));
        }
        else return aiosc_response(0,__('No se seleccionaron prioridades por lo que no se tomó ninguna acción.','aiosc'));
    }
    ...
    static function install() {
        ...
        if(!is_array($objs)) {
            self::update_priority(0,
                __('Sin Prioridad','aiosc'),
                __('Esta es la prioridad predeterminada creada en la instalación de Ticket System.','aiosc'),
                0,null,true,
                true);
        }
    }
