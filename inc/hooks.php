...
class aiosc_Hooks {
    ...
    function toolbar_new_tickets($wp_admin_bar) {
        ...
        if($cnt > 0) {
            $args = array(
                'id'    => 'aiosc',
                'title' => '<span class="ab-icon" title="'.sprintf(__('Tienes %d tickets nuevos en la cola.','aiosc'),$cnt).'"><i class="dashicons-before dashicons-tickets"></i></span><span class="ab-label">'.$cnt.'</span>',
    ...
    function preferences_save() {
        ...
        if(isset($_POST['section']) && in_array($_POST['section'], $sections)) {
            ...
            echo aiosc_response(1,__('Los ajustes se guardaron correctamente.','aiosc'));
            die();
        }
    ...
    function account_save() {
        ...
        if(isset($_POST['section']) && in_array($_POST['section'], $sections)) {
            ...
            echo aiosc_response(1,__('La configuración de la cuenta se guardó correctamente.','aiosc'));
            die();
        }
        else {
            echo aiosc_response(0,__("Cheatin' huh?.",'aiosc'));
